...
public class ReadOrderLambda {
    ...
    public APIGatewayProxyResponseEvent readOrders(final APIGatewayProxyRequestEvent input,
                                                    final Context context)
            throws JsonProcessingException {

        ScanResult scanResult =
                dynamoDB.scan(new ScanRequest().withTableName(System.getenv("ORDERS_TABLE")));

        List<Order> listOfOrdersFetchedFromDb = scanResult.getItems().stream().map(item ->
        ...
        String outputJson = objectMapper.writeValueAsString(listOfOrdersFetchedFromDb);
